...
## ⇢ RE-SCOPED 2026-08-26 (owner directive): a LIVE, fully-demonstrable MVP — ASAP

**Owner, in session, 2026-08-26: the aim is "getting my MVP site that is 'fully demonstratable'
live as soon as possible."** This supersedes the 2026-08-07 local-MVP target below WITHOUT
shrinking it: everything the local scope required is still required (it is the "fully
demonstrable" half), and the deploy tail comes back ON the path. What changes:

- **Track D → UN-ICED.** D1 preview+prod serving loads (runbook exists; H1's fixed
  `apply_serving_schema.py` is the mandated path, ordered BEFORE `apply_auth_schema.py`),
  D2 solver deploy, D3 access grants.
- **"Live" means DEMOABLE, not open**: the site stays behind the `early_access` gate; the finish
  line is the owner granting access and demoing the full experience from fundscore.ai. Invite
  policy beyond that stays an owner call at S5 (reframed: demo go/no-go).
- **S4 is live again and CHANGED by the 2026-08-21 EODHD ruling** — what needs confirming before
  any solver image push is the licence posture of whatever the baked snapshot derives from TODAY
  (still Sharadar/Tiingo until the EODHD migration lands), not the original framing.
- **P1-preview** (Supabase paid tier for preview) is a live decision again — it gates D2's AC3.
- The 2026-08-07 definition of done below still holds as the gate in front of S3; F6 cutover then
  feeds D1→D2→D3 instead of ending the line.
...
| F3 | Recent Changes section flip | IN | opus/high | **done 2026-08-26** — flip shipped + dispatcher-verified 00:13, codex clean (`845f65d`), merged to web main `18a955e`. Page-level critic read folded into F5 (same as F2's). |
...
| F7 | Screener rebuild on `fund_profile_facts` (owner 2026-08-08, P2 answered: BUILD for the demo) — backlog story; spec must settle overlap with `exposure-screener` + tier columns | SS→IN (standard) | opus/high | **done 2026-08-26** — `0b5839c` on `f7/screener-rebuild`, merged to web main `18a955e`; fabricated demo cluster DROPPED the same morning (register: standing actions). Renders from `fund_profile_facts`; empty on prod until D1 loads serving data. |

### Track D — deploy + beta — **UN-ICED 2026-08-26 (owner directive: live fully-demonstrable MVP ASAP; see the dated re-scope at top).** D1 stays sequenced behind the blocker bar + F6; S4/S5 and the preview-tier decision are live again. *(Was: ICED 2026-08-07, "not on the local-MVP path" — that target is superseded.)*
| # | Item | Worker | Model/effort | STATUS |
|---|------|--------|--------------|--------|
...
- 2026-08-26 10:52 — **STOCK-TAKE + CLEANUP SESSION (owner present) — SCOPE FLIPS TO LIVE.**
  ...
  as possible."* Track D un-iced (dated re-scope at top; S5 reframed as demo go/no-go behind the
  `early_access` gate). fundscore.ai checked: answers 200.

  **Cleanup, each removal verified first:**
  - **fund_score:** 8 merged-but-dirty worktrees removed (dirt adjudicated: regenerated check-data
    reports superseded on main, a `.venv`, a scratchpad); 5 worktrees remain — main + 4
    owner-parked (capgain / d83 / l10 / l9). 16 merged branches deleted with `-d`. 3 stranded
    codex verdicts (`9378de7`, `53537b2`, `009b872`) rescued from dying worktrees into
    `feature-pipeline/reviews/` — **UNCOMMITTED on main; sweep them into the next fund_score
    branch commit.** Both stashes verified superseded (stash@{0}'s `management_structure` work is
    fully on main; stash@{1}'s deletions long since landed) but `git stash drop` was
    permission-blocked — owner one-liner owed.
  - **fundscore-web:** worktree `wt-f1` removed; 10 merged branches deleted. Two need an owner
    `-D`/push-delete (force-delete permission-blocked): `feature/crescent-profile-v2` (merged to
    main; only its origin tracking ref lags) and `day-close/20260717` (fully redundant — its one
    commit duplicates main's `d49d99b`, byte-identical files).
  - **fundscore-harness:** `fix/commit-hook-target-resolution` was already merged — deleted.
    `chore/backlog-finalize-in-loops` (`c0b314e`, 9 lines: fix-loops archive the closed backlog
    block at finalize) is real and unmerged — **awaits owner merge.**

  **Status rows corrected in this file** (stale rows misdirect dispatchers — same failure class
  the 2026-08-25 23:40/23:55 corrections fixed): L6 done · F3 done · F7 done · L13 unblocked
  (with the v0.5 registry-bump reload coupling) · Track D un-iced.

  **Specced for the next session:** `positioning-isin-rekey` (decision B — the ~1M-row
  matching-key change; Segment 0 = what-else-moves measurement, zero canonical writes, owner
  review before any build) → `specs/queue/`.

  **Owner decision batch assembled and delivered in-session** (L10's five · L9 D-1 · the
  `076562f` d83 merge ruling · P5-1/P5-1a · P1-preview · S4-as-amended-by-EODHD · demo-access
  policy · harness chore merge). Rulings land in the register, not here.
